USE `orbs_token`;
DROP procedure IF EXISTS `get_guardians_excellence_aggregate`;

DELIMITER $$
USE `orbs_token`$$
CREATE DEFINER=`orbs`@`%` PROCEDURE `get_guardians_excellence_aggregate`(IN election_num INTEGER)
BEGIN
...
SELECT 
    KNOWN(address), address, COUNT(address) AS elegible_count
FROM
    tmp_excellence
GROUP BY address;
DROP TEMPORARY TABLE IF EXISTS tmp_excellence;
